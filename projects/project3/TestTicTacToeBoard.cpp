...
// , , ,
        ...
        if (type == 'C') {
           ...
           for (int row = 0; row < tttBoard->size() && !foundNonSpace; row++) {
               for (int col = 0; col < tttBoard->size() && !foundNonSpace; col++) {
                   if (tttBoard->getValue(row, col) != ' ') {
                      std::cout << "clear method is not correct. Each element should be a space character\n";
                      foundNonSpace = true; //set a tag
...
//TODO: need to finish mark method of TicTacToeBoard.cpp
TicTacToeBoard* assignData(char data[][NUM_COLUMNS]) {
